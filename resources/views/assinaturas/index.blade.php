        ...
        @foreach($planos as $plano)
            ...
            <div @class([
                ...
                <div class="mt-6">
                    ...
                    @else
                        <form action="{{ route('assinaturas.assinar') }}" method="POST"
                              x-data="{ loading: false }" @submit="loading = true">
                            @csrf
                            <input type="hidden" name="plano_id" value="{{ $plano->id }}">

                            <button type="submit" :disabled="loading" :aria-busy="loading"
                                    class="w-full inline-flex items-center justify-center gap-2 min-h-[44px] rounded-lg bg-entrego-blue text-white py-3 font-medium hover:bg-entrego-blue-600 focus:outline-none focus:ring-2 focus:ring-entrego-blue focus:ring-offset-2 disabled:opacity-70 disabled:cursor-not-allowed transition-colors">
                                ...
                                <span x-show="!loading">{{ $assinaturaAtual ? 'Trocar para este plano' : 'Escolher plano' }}</span>
